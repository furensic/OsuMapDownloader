...
namespace OsuMapDownloader {
    internal class Program {
        static void Main(string[] args) {
            ...
            osuAuth.redirectUri = "http://127.0.0.1:9000/"; // set redirect url 

            // get auth url and display it in console
            var authorizationUrl = osuAuth.GetAuthorizationUrl();
            Console.WriteLine(authorizationUrl);

            // start the oauth flow
            osuAuth.StartOauthFlow();
        }
    }
    ...
    public class OsuAuthentication {
        ...
        public OsuAuthorizationCodeGrant GetAccessToken(string _code) {
            ...
            return client.Post<OsuAuthorizationCodeGrant>(request);
        }
    }
